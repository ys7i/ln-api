...
// SchemaMigration represents a row from 'db.schema_migrations'.
...
func (sm *SchemaMigration) Insert(ctx context.Context, db DB) error {
	...
	// insert (manual)
	const sqlstr = `INSERT INTO db.schema_migrations (` +
		`version, dirty` +
		`) VALUES (` +
		`?, ?` +
		`)`
...
func (sm *SchemaMigration) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE db.schema_migrations SET ` +
		`dirty = ? ` +
		`WHERE version = ?`
	// run
...
func (sm *SchemaMigration) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM db.schema_migrations ` +
		`WHERE version = ?`
	// run
...
// SchemaMigrationByVersion retrieves a row from 'db.schema_migrations' as a SchemaMigration.
//
